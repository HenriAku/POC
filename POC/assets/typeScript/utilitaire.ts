...
const Card = {
	setup() {
		const nom    = ref<string[]>(["Alice", "Bob", "Charlie", "David", "Eve"]);
		const img    = ref<string> ('./assets/images/logo_salamander_esport.svg');
		const taille = ref<string>(null);
		const ind    = Math.floor(Math.random()*nom.value.length);
		const desc   = ref<string>(`Vend ${nom.value[ind]} en biengin`);

		const changementImage = (newImg : string) :void => {
		  img.value = newImg;
		};

		const getTaille = ():string => {
			if(taille.value == null) return 'Tailles';
			return taille.value;
		};

		const changementTaille = (newTaille:string) :void => {
		...
		const getImageInfos = () :string => {
			return img.value;
		};

		const getRandomNom = ():string => {
		...
		const getInfos = ():string => {
			return `
			<p>${getRandomNom()}</p>
			<p>${getDescription()}</p>
			<p>Taille : ${taille.value ?? "Taille pas spécifiée"}</p>
			`;
		};
